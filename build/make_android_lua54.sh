...
function build() {
    ...
    cmake -H. -B${BUILD_PATH} -DLUAC_COMPATIBLE_FORMAT=ON -DLUA_VERSION=5.4.1 -DANDROID_ABI=${ABI} -DCMAKE_TOOLCHAIN_FILE=${NDK}/build/cmake/android.toolchain.cmake -DANDROID_NATIVE_API_LEVEL=${API} -DANDROID_TOOLCHAIN=clang -DANDROID_TOOLCHAIN_NAME=${TOOLCHAIN_ANME} -DANDROID_STL=c++_shared
